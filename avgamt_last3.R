...
res<-res[,.(Avg_amount_last3=mean(head(Amount,3))),by=Mobile]

z = quantile(res$Avg_amount_last3,c(0.2,0.6),na.rm=TRUE)
y <- data.frame(id = c(0.2,0.6), values = z) 
quan_val<-y[,2]

#Function
res$Type<-NA
res$Type<-sapply(res$Avg_amount_last3,fun)
fun<-function(y)
{
  if(y<quan_val[1])
    return("Bad")
...
#alter final
dbGetQuery(con,statement='update supermaster f inner join temptab t on t.mobile = f.mobile set f.Avg_amount_last3 = t.Type')
