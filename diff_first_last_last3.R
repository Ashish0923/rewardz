#Getting Data
no_days<-dbGetQuery(con,statement='select Mobile,Dt from sku_data where (Mobile!=\'NULL\' and SUB_CATEGORY!=\'CARRY BAG\' and DEPARTMENT!=\'NULL\' and Qty>0)')
no_days<-data.table(no_days)

#Processing Data
no_days<-no_days[order(Mobile,-rank(Dt))]
no_days<-no_days[!duplicated(no_days), ]

#Function
no_days<-no_days[, .(Date=head(Dt,3)), by = list(Mobile)]
...
#alter final
dbGetQuery(con,statement='update supermaster f inner join temptab t on t.mobile = f.mobile set f.diff_last3 = t.No_days')
