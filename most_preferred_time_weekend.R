...
#
tab<-most_preferred_time[type=="weekend customer"]
tab$Time<-NA
...
tab<-tab[,.(WeekendAvg_time=mean(Time,na.rm=TRUE)),by=Mobile]
z = quantile(as.numeric(tab$WeekendAvg_time),c(0.2,0.6),na.rm=TRUE)
...
tab$Type<-sapply(is.numeric(tab$WeekendAvg_time),exp)
...
#alter final
dbGetQuery(con,statement='update supermaster f inner join temptab t on t.mobile = f.mobile set f.Most_preferred_time_Weekend = t.Type')
